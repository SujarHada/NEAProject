...
export const branchCreationSchema = (t: (key: string) => string) => {
	return z.object({
		...
		email: z
			.email({ message: t("createBranch.validation.emailInvalid") })
			.min(1, t("createBranch.validation.email")),
...
export const branchUpdateSchema = (t: (key: string) => string) => {
	return z.object({
		...
		email: z
			.email({ message: t("editBranch.validation.emailInvalid") })
			.min(1, t("editBranch.validation.email")),
