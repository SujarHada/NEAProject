...
const ActiveProducts = () => {
	...
	const handleDownload = async () => {
		...
		link.setAttribute(
			"download",
			`active_products_${new Date().toISOString()}.csv`,
		);
					...
					<thead className="text-xs uppercase bg-gray-700 text-gray-400 border-b">
						<tr>
							<th className="px-6 py-3">
								{" "}
								{t("activeProductsPage.table.sn")}{" "}
							</th>
							<th className="px-6 py-3">
								{" "}
								{t("activeProductsPage.table.sku")}{" "}
							</th>
							<th className="px-6 py-3">
								{" "}
								{t("activeProductsPage.table.name")}{" "}
							</th>
							<th className="px-6 py-3">
								{" "}
								{t("activeProductsPage.table.company")}{" "}
							</th>
							<th className="px-6 py-3">
								{" "}
				...
				<ul className="flex items-center justify-center h-8 text-sm">
					<li>
						<button
							type={"button"}
							onClick={() =>
								prevPage && fetchProducts(prevPage, currentPage - 1)
							}
							disabled={!prevPage}
							className={`flex items-center justify-center px-3 h-8 border rounded-s-lg bg-gray-800 border-gray-700 ${
								prevPage
					...
					{Array.from({ length: totalPages }).map((_, i) => (
						<li key={i}>
							<button
								...
								className={`flex items-center justify-center px-3 h-8 border bg-gray-800 border-gray-700 ${
									currentPage === i + 1
					...
					<li>
						<button
							type={"button"}
							onClick={() =>
								nextPage && fetchProducts(nextPage, currentPage + 1)
							}
							disabled={!nextPage}
							className={`flex items-center justify-center px-3 h-8 border rounded-e-lg bg-gray-800 border-gray-700 ${
								nextPage
									? "text-gray-400 hover:bg-gray-700 hover:text-white"
									: "text-gray-600 cursor-not-allowed"
							}`}
