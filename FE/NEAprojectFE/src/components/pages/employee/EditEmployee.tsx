...
import type { Employee, Branch } from "app/interfaces/interfaces";
...
import {
	updateEmployeesFormSchema,
	type UpdateEmployeesFormData,
} from "app/schemas/employee";
...
const EditEmployee = () => {
	...
	useEffect(() => {
		if (param.id) {
			const branch = branches?.find(
				(branch) => branch.organization_id === employee?.organization_id,
			);
			if (branch) {
	...
	const {
		...
		setValue,
		reset,
	} = useForm<UpdateEmployeesFormData>({
		defaultValues: {
			first_name: "",
			middle_name: "",
			last_name: "",
		...
		<div className="flex flex-col gap-6">
			...
			<div className="flex flex-col lg:w-1/2 gap-2">
				<label htmlFor="organization_id">
					{t("editEmployee.labels.branchId")} *
				</label>
				<Controller
						...
						<div className="flex w-full items-center relative">
							...
							>
								<option value="" disabled hidden>
									{t("createEmployee.placeholders.branches")}
								</option>
								{branches.map((branch) => (
									<option key={branch.id} value={branch.organization_id}>
										{branch.name}
									</option>
								))}
							</select>
			...
			<div className="flex w-full flex-wrap gap-4">
				<div className="flex flex-1 flex-col gap-2">
					<label htmlFor="first_name">
						{t("editEmployee.labels.firstName")} *
					</label>
					<Controller
						name="first_name"
						control={control}
				...
				<div className="w-full flex-1 flex flex-col gap-2">
					<label htmlFor="middle_name">
						{t("editEmployee.labels.middleName")}
					</label>
					<Controller
						name="middle_name"
						control={control}
				...
				<div className="w-full flex flex-1 flex-col gap-2">
					<label htmlFor="last_name">
						{t("editEmployee.labels.lastName")} *
					</label>
					<Controller
						name="last_name"
						control={control}
			...
			<div className="flex w-full flex-wrap gap-4">
				<div className="lg:w-1/2 flex flex-1 flex-col gap-2">
					<label htmlFor="password">
						{t("editEmployee.labels.password")} *
					</label>
					<Controller
						name="password"
						control={control}
				...
				<div className="lg:w-1/2 flex flex-1 flex-col gap-2">
					<label htmlFor="password">
						{t("editEmployee.labels.confirmPassword")} *
					</label>
					<Controller
				...
				<Controller
					...
					render={({ field }) => (
						<div className="flex w-full items-center relative">
							...
							>
								<option value="" disabled hidden>
									{t("editEmployee.placeholders.position")}
								</option>
								<option value="admin">
									{t("editEmployee.positions.admin")}
								</option>
								<option value="viewer">
			...
			<div className="flex">
				...
				>
					{isSubmitting
						? t("editEmployee.button.saving")
						: t("editEmployee.button.save")}
				</button>
			</div>
		</div>
	);
